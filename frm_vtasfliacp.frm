...
Begin VB.Form frm_vtasfliacp 
   ...
   Begin VB.CommandButton b_canc 
      ...
      Height          =   495
      Left            =   5400
      MouseIcon       =   "frm_vtasfliacp.frx":0442
      MousePointer    =   99  'Custom
      Picture         =   "frm_vtasfliacp.frx":074C
      Style           =   1  'Graphical
      TabIndex        =   9
      ToolTipText     =   "Salida"
      Top             =   4920
   ...
   Begin VB.CommandButton b_acep 
      ...
      Height          =   495
      Left            =   360
      MouseIcon       =   "frm_vtasfliacp.frx":0CD6
      MousePointer    =   99  'Custom
      Picture         =   "frm_vtasfliacp.frx":0FE0
      Style           =   1  'Graphical
      TabIndex        =   8
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "frm_vtasfliacp.frx":156A
         Left            =   2640
         List            =   "frm_vtasfliacp.frx":1580
      ...
      Begin VB.Data data_med 
         ...
         ReadOnly        =   0   'False
         RecordsetType   =   1  'Dynaset
         RecordSource    =   "select * from familias where fam_numero in (1,2,10,14,16,18)"
         Top             =   1200
         Visible         =   0   'False
         Width           =   2535
      End
      ...
      Begin MSDBCtls.DBCombo DBCombo1 
         Bindings        =   "frm_vtasfliacp.frx":15C0
         Height          =   360
         Left            =   1560
         TabIndex        =   5
         Top             =   960
   ...
   Begin VB.Image Image1 
      Height          =   1095
      Left            =   2280
      Picture         =   "frm_vtasfliacp.frx":15D7
      Stretch         =   -1  'True
      Top             =   4680
      Width           =   1815
   End
End
Attribute VB_Name = "frm_vtasfliacp"
         ...
         If txt_b.Text <> "" Then
            If txt_b.Text = 99 Then
               If Check2.Value = 1 Then
                  data_lin.RecordSource = "Select * from resplin where fecha >='" & Format(md.Text, "yyyy-mm-dd") & "' And fecha <='" & Format(mh.Text, "yyyy-mm-dd") & "' and nro_flia =" & tm.Text & " and convenio not in ('SMIN','CCNOS','UNIVS','HEVANO','GANOS','CASANO','PART','SMI4','CCSD','UNIDI') and tipo <>'" & "NOTA CR" & "' and cod_prod not in (999,992,800,881,882,991,993,994,996,997,8000,995,30,31) and nro_flia not in (19,6) order by cod_prod"
                  data_lin.Refresh
               Else
                  data_lin.RecordSource = "Select * from linmmdd where fecha >='" & Format(md.Text, "yyyy-mm-dd") & "' And fecha <='" & Format(mh.Text, "yyyy-mm-dd") & "' and nro_flia =" & tm.Text & " and convenio not in ('SMIN','CCNOS','UNIVS','HEVANO','GANOS','CASANO','PART','SMI4','CCSD','UNIDI') and cod_prod not in (999,992,800,881,882,991,993,994,996,997,8000,995,30,31) and nro_flia not in (19,6) order by cod_prod"
                  data_lin.Refresh
               End If
            Else
               If Check2.Value = 1 Then
                  data_lin.RecordSource = "Select * from resplin where fecha >='" & Format(md.Text, "yyyy-mm-dd") & "' And fecha <='" & Format(mh.Text, "yyyy-mm-dd") & "' and nro_flia =" & tm.Text & " And base =" & txt_b.Text & " And convenio not in ('SMIN','CCNOS','UNIVS','HEVANO','GANOS','CASANO','PART','SMI4','CCSD','UNIDI') and tipo <>'" & "NOTA CR" & "' and cod_prod not in (999,992,800,881,882,991,993,994,996,997,8000,995,30,31) and nro_flia not in (19,6) order by cod_prod"
                  data_lin.Refresh
               Else
                  data_lin.RecordSource = "Select * from linmmdd where fecha >='" & Format(md.Text, "yyyy-mm-dd") & "' And fecha <='" & Format(mh.Text, "yyyy-mm-dd") & "' and nro_flia =" & tm.Text & " And base =" & txt_b.Text & " And convenio not in ('SMIN','CCNOS','UNIVS','HEVANO','GANOS','CASANO','PART','SMI4','CCSD','UNIDI') and cod_prod not in (999,992,800,881,882,991,993,994,996,997,8000,995,30,31) and nro_flia not in (19,6) order by cod_prod"
                  data_lin.Refresh
               End If
            End If
               ...
               Do While Not data_lin.Recordset.EOF
                  ...
                  If data_conv.Recordset.RecordCount > 0 Then
                     If IsNull(data_conv.Recordset("cnv_grupo")) = False Then
                        If data_conv.Recordset("cnv_grupo") = "UNIVERSAL" Or _
                           data_conv.Recordset("cnv_grupo") = "CCOU" Or _
                           ...
                           data_conv.Recordset("cnv_grupo") = "IMPASA" Then
